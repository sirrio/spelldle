...
export default function Home() {
  ...
  return (
    <main>
      ...
      {showHow && <div className="modal-backdrop" onMouseDown={() => setShowHow(false)}><div className="modal" role="dialog" aria-modal="true" aria-labelledby="how-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setShowHow(false)} aria-label="Close">×</button><div className="panel-kicker">HOW TO PLAY</div><h2 id="how-title">Find the spell</h2><p>You have six guesses. Each spell is compared by level, school, components, ritual casting, casting time, range and duration.</p><ol><li>Choose a spell from the archive.</li><li>Green is an exact match; yellow is a partial match.</li><li>For level, range and duration, the arrow points toward the target value.</li></ol><div className="legend modal-legend"><span><i className="swatch exact" />Exact match</span><span><i className="swatch partial" />Partial match</span><span><i className="swatch wrong" />No match</span></div><div className="credits"><strong>ICON CREDITS</strong><p>Spell icons by Lorc, Delapouite and the contributors of <a href="https://game-icons.net/" target="_blank" rel="noreferrer">Game-icons.net</a>, used under <a href="https://creativecommons.org/licenses/by/3.0/" target="_blank" rel="noreferrer">CC BY 3.0</a>.</p></div></div></div>}
